...
// Simple list of items to sort
const simpleList = ["oranges", "grapes", "lemons", "apples", "Bananas", "watermelons", "coconuts", "broccoli", "mango"];

// Function to compare values for sorting in descending order
...
// Sort simpleList in descending order
const sortedSimpleList = simpleList.sort(compareFn);
console.log("Sorted Simple List (Descending):", sortedSimpleList);

// Sorting hikes by name in descending order
...
console.log("Sorted Hikes (Descending by Name):", sortedHikes);

// Function to search the hikes list and display results
function performSearch() {
    const query = document.getElementById("searchInput").value.toLowerCase();
    const results = hikes.filter(hike => hike.name.toLowerCase().includes(query));

    document.getElementById("output").innerHTML = results.length
        ? results.map(hike => `<p><strong>${hike.name}</strong>: ${hike.description}</p>`).join("")
        : "No matches found";
}
// Example test cases (you can test this in the console or use in real time)
